...
# -------------------- SESLİ KONUŞMA SİSTEMİ (EMEL SESİ) --------------------
async def sesli_soyle(kanal, metin):
    """Edge TTS - Emel sesiyle Türkçe konuşma"""
    try:
...
async def on_ready():
    ...
    print(f"🎙️ Sesli karşılama aktif! Ses: Emel (tr-TR-EmelNeural)")
    print(f"👥 Karşılama rolleri: {KARSILAMA_ROLLERI}")
...
async def ses_kanalina_gir(ctx, *, kanal_adi: str = None):
    """Botu ses kanalına sokar. .gir -> V.Confirmed / .gir <kanal>"""
...
async def ses_bilgi(ctx):
    """Botun şu anki ses durumunu gösterir"""
    if ctx.voice_client is None:
        embed = discord.Embed(
            title="🔇 Ses Durumu",
...
async def sesli_soyle_komut(ctx, *, metin: str):
    """Yazdığın metni Emel sesiyle söyler"""
    if not ctx.author.voice:
        return await ctx.send("❌ Ses kanalında değilsin!")
    
    await ctx.send(f"🎙️ Emel söylüyor: *{metin[:100]}*")
...
async def erkek_kayit(ctx, member: discord.Member, ad, yas):
    """.e @kullanıcı Ad Yaş - Erkek kaydı"""
...
async def kayit_liderlik(ctx):
    """En çok kayıt yapan yetkilileri gösterir"""
    veri = veri_yukle()
...
async def dogum_gunu_ekle(ctx, gun: int, ay: int):
    """Doğum günü ekle: .dogumgunu 15 7 (15 Temmuz)"""
...
async def dogum_gunleri_liste(ctx):
    """Kayıtlı doğum günlerini gösterir"""
    veri = veri_yukle()
...
async def kayit_istatistik(ctx):
    """Günlük ve haftalık kayıt istatistikleri"""
    veri = veri_yukle()
    kayitlar = veri.get("kayitlar", {})
    simdi = datetime.now()
    bugun = simdi.date()
...
async def davetlerim(ctx, member: discord.Member = None):
    """Davet istatistiklerini gösterir"""
    if member is None:
        member = ctx.author
    veri = veri_yukle()
...
async def davet_liderlik(ctx):
    """En çok davet edenleri gösterir"""
    veri = veri_yukle()
...
async def sohbet_baslat(ctx):
    """Sohbet başlatmak için buz kırıcı soru"""
    sorular = [
...
async def muhabbet(ctx, *, konu: str = None):
    """Belirli bir konuda muhabbet başlatır"""
    if konu is None:
...
async def yardim(ctx):
    ...
    embed.add_field(
        name="🎙️ **YENİ! Sesli Karşılama (Emel)**",
        value="`Kayıtsız` veya `Founder Of Kross` rolüne sahip kişiler ses kanalına katıldığında bot otomatik olarak **Emel** sesiyle hoş geldin mesajı söyler!",
        inline=False
    )
    await ctx.send(embed=embed)
...
if __name__ == "__main__":
    ...
    print("🎙️ Seslendirme: Emel (tr-TR-EmelNeural) - Doğal Kadın Sesi")
    print("👥 Otomatik karşılama rolleri: Kayıtsız, Founder Of Kross")
