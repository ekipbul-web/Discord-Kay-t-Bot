...
class SesYoneticisi:
    ...
    async def kanala_katil(self, ctx, kanal_adi):
        ...
        if kanal is None:
            # Tam eşleşme yoksa içinde geçen var mı bak
            for vc in ctx.guild.voice_channels:
...
async def on_voice_state_update(member, before, after):
    """Bot ses kanalından atılırsa otomatik geri dön"""
    
    # Sadece bot için kontrol
    if member.id != bot.user.id:
        return
    
    # Bot zorla çıkarıldıysa veya kanal silindiyse
...
async def on_member_join(member):
    ...
    embed = discord.Embed(
        ...
        description=f"**Ress Sunucumuza Hoş Geldin, {member.mention}!**",
    ...
    embed.set_footer(text=f"Ress Sunucusu • {member.guild.name}")
...
async def kayit_mesaji_gonder(ctx, member, kayit_eden, verilen_rol):
    ...
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text=f"Ress Sunucusu • Toplam Üye: {ctx.guild.member_count}")
...
async def ses_kanalina_gir(ctx, *, kanal_adi: str = None):
    ...
    .gir -> V.Confirmed kanalına girer
    .gir <kanal_adı> -> İstediğin kanala girer
    
    NOT: Ses kanalında olmanıza gerek yok! Direkt çalışır.
    """
...
async def dogum_gunu_ekle(ctx, gun: int, ay: int):
    """Doğum gününü ekle: .dogumgunu 15 7 (15 Temmuz)"""
...
async def dogum_gunleri_liste(ctx):
    """Yaklaşan doğum günlerini gösterir"""
    veri = veri_yukle()
...
async def sohbet_baslat(ctx):
    """Sohbet başlatmak için buz kırıcı sorular"""
    sorular = [
